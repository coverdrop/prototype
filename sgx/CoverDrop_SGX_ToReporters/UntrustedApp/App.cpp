...
#define ENCLAVE_FILE _T("CoverDrop_SGX.signed.dll")
...
int main(){
	...
	std::string getURL = "/user_messages?count=1"; //First, get one message to get length of messages
	uint8_t procmsg = 0;
	...
	ret = clean_arrays(eid, &innerret);
	if (ret != SGX_SUCCESS || innerret != SGX_SUCCESS) {
		printf("\nERROR CLEANING ARRAYS\n");
		getchar();
		return -1;
	}
	getURL = "/user_messages?count=50";
	while (true) {
		j = get_messages(getURL);
		...
		for (int i = 0; i < j.size(); i++) {
			...
			std::string delURL = "/user_message/" + boost::lexical_cast<std::string>(id);
			int retval = 1;
...
//TODO: make this into batched process. One call for all messages for a particular RID
int transmit_message_to_reporter(const char *str, size_t len, uint8_t RID) {
	int retval = 0;
	try {
		auto const* host = "coverdrop.cl.cam.ac.uk";
		auto const* port = "80";
		std::string target_user = "/send_to_reporter/";
		target_user = target_user + (char)(RID+'0');
		int version = 11;
		// The io_context is required for all I/O
		net::io_context ioc;
